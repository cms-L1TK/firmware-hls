...
TrackletProcessor_parameters.h in the TopFunctions/ directory.",
                                 epilog="")
parser.add_argument("-o", "--outputDirectory", metavar="DIR", default="../TopFunctions/CombinedConfig/",
                    type=str, help="The directory in which to write the output files (default=%(default)s)")
parser.add_argument("-w", "--wiresFileName", metavar="WIRES_FILE", default="LUTsCM/wires.dat",
                    type=str, help="Name and directory of the configuration file for wiring (default = %(default)s)")
...
with open(os.path.join(dirname, arguments.outputDirectory, "TrackletProcessor_parameters.h"), "w") as parametersFile, \
    ...
    for tpName in sorted(asInnerMems.keys()):
        ...
        if seed not in seedlist:
            LUTSize[seed] = str(
                sum(1 for _ in open("LUTsCM/TP_{0}.tab".format(seed))) - 2)
            regLUTSize[seed] = str(
                sum(1 for _ in open("LUTsCM/TP_{0}{1}_usereg.tab".format(seed, iTC))) - 2)
            innerPTLUTSize[seed] = str(sum(1 for _ in open(
                "LUTsCM/TP_{0}{1}_stubptinnercut.tab".format(seed, iTC))) - 2)
            outerPTLUTSize[seed] = str(sum(1 for _ in open(
                "LUTsCM/TP_{0}{1}_stubptoutercut.tab".format(seed, iTC))) - 2)
        # Print out parameters for this TP.
        parametersFile.write(
            ("\n"
             "template<> constexpr uint32_t TPROJMaskBarrel<TF::" +
             seed + ", TC::" + iTC + ">() {\n"
             "  return 0x%X;\n"
             "}\n"
             "template<> constexpr uint32_t TPROJMaskDisk<TF::" +
             seed + ", TC::" + iTC + ">() {\n"
             "  return 0x%X;\n"
             "}\n"
             'template<> inline const ap_uint<10>* getLUT<TF::' +
             seed + ', TC::' + iTC + ' >(){\n'
             '#ifndef __SYNTHESIS__\n'
             '#ifdef CMSSW_GIT_HASH\n'
             ...
             '  static bool init = 0;\n'
             '  if (!init)\n'
             '    init = readSWLUT<ap_uint<10>,' +
             LUTSize[seed] + '>(lut,"LUTsCM/TP_' + seed + '.tab");\n'
             '#else\n'
             ...
             '  return lut;\n'
             '}\n'
             'template<> inline const ap_uint<1>* getPTInnerLUT<TF::' +
             seed + ', TC::' + iTC + ' >(){\n'
             '#ifndef __SYNTHESIS__\n'
             '#ifdef CMSSW_GIT_HASH\n'
             '  static std::mutex getLUTMutex_;\n'
             ...
             '  static bool init = 0;\n'
             '  if (!init)\n'
             '    init = readSWLUT<ap_uint<1>,' +
             innerPTLUTSize[seed] + ' >(lut,"LUTsCM/TP_' +
             seed + iTC + '_stubptinnercut.tab");\n'
             '#else\n'
             '  static ap_uint<1> lut[] =\n'
             '#if __has_include("../emData/LUTsCM/TP_' +
             seed + iTC + '_stubptinnercut.tab")\n'
             ...
             '  return lut;\n'
             '}\n'
             'template<> inline const ap_uint<1>* getPTOuterLUT<TF::' +
             seed + ', TC::' + iTC + ' >(){\n'
             '#ifndef __SYNTHESIS__\n'
             '#ifdef CMSSW_GIT_HASH\n'
             '  static std::mutex getLUTMutex_;\n'
             ...
             '  if (!init)\n'
             '    init = readSWLUT<ap_uint<1>,' +
             outerPTLUTSize[seed] + '>(lut,"LUTsCM/TP_' +
             seed + iTC + '_stubptoutercut.tab");\n'
             '#else\n'
             '  static ap_uint<1> lut[] =\n'
             '#if __has_include("../emData/LUTsCM/TP_' +
             seed + iTC + '_stubptoutercut.tab")\n'
             ...
             '  return lut;\n'
             '}\n'
             'template<> constexpr int nASMemInner<TF::' +
             seed + ', TC::' + iTC + '>(){\n'
        ...
        topHeaderFile.write(
            ...
            "    const BXType bx,\n"
            "    BXType& bx_o,\n"
            "    const AllStubInnerMemory<InnerRegion<TF::" +
            str(seed) + ">()> innerStubs[nASMemInner<TF::" +
            str(seed) + ", TC::" + iTC + ">()],\n"
            "    const AllStubMemory<OuterRegion<TF::" +
            str(seed) + ">()>* outerStubs,\n"
            "    const VMStubTEOuterMemoryCM<OuterRegion<TF::" +
            str(seed) +
            ">(), kNbitsrzbin, kNbitsphibin, kNTEUnitsLayerDisk[TF::" +
            seed[2:]+"]>* outerVMStubs,\n"
        ...
        for i in range(0, nASMemInner):
            topFile.write(
                "#pragma HLS resource variable=innerStubs[" + str(i) + "].get_mem() latency=2\n")
        topFile.write(
