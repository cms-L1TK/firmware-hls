...
module AESL_automem_innerStubs_dataarray_data_V (
    clk,
    rst,
    ce0,
    we0,
...
//------------------------Parameter----------------------
localparam
    TV_IN  = "../tv/cdatafile/c.TrackletProcessor_L2L3C.autotvin_innerStubs_dataarray_data_V.dat", 
    TV_OUT = "../tv/rtldatafile/rtl.TrackletProcessor_L2L3C.autotvout_innerStubs_dataarray_data_V.dat"; 
//------------------------Local signal-------------------
parameter DATA_WIDTH = 32'd 51;
parameter ADDR_WIDTH = 32'd 12;
parameter DEPTH = 32'd 3072;
parameter DLY = 0.1;
...
always @ (posedge clk or rst) begin
    if(rst === 1) begin
        ...
        if((we0 == 0) && (ce0 == 1) && (ce1 == 1) && (we1 == 1) && (address0 == address1)) begin
            dout0_delay1 <= #DLY din1;
            dout0 <= #DLY dout0_delay1;
        end
        else if(ce0 == 1) begin
            dout0_delay1 <= #DLY mem[address0];
            dout0 <= #DLY dout0_delay1;
        end
        else ;
    end
...
always @ (posedge clk or rst) begin
    if(rst === 1) begin
        ...
        if((we0 == 1) && (ce0 == 1) && (ce1 == 1) && (we1 == 0) && (address0 == address1)) begin
            dout1_delay1 <= #DLY din0;
            dout1 <= #DLY dout1_delay1;
        end
        else if(ce1 == 1) begin
            dout1_delay1 <= #DLY mem[address1];
            dout1 <= #DLY dout1_delay1;
        end
        else ;
    end
