...
void VMRouterCMTop_L1PHIB(
  ...
  static const int* METable = getMETable<layerdisk_B>();
  static const int* TETable = getTETable<layerdisk_B>();

  // LUT with phi corrections to project the stub to the average radius in a layer.
  // Only used by layers.
  // Indexed using phi and bend bits
  static const int* phiCorrTable = getPhiCorrTable<layerdisk_B>();
