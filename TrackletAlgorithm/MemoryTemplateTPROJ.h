...
#ifdef CMSSW_GIT_HASH
#define NBIT_BX 0
template<class DataType, unsigned int DUMMY, unsigned int NBIT_ADDR, unsigned int NPAGE>
#else
template<class DataType, unsigned int NBIT_BX, unsigned int NBIT_ADDR, unsigned int NPAGE>
#endif
...
#pragma HLS ARRAY_PARTITION variable=nentries_ complete dim=0
  ...
  const DataType (&get_mem() const)[1<<NBIT_BX][NPAGE][1<<NBIT_ADDR] {return dataarray_;}

  DataType read_mem(BunchXingT ibx, ap_uint<NBIT_ADDR> index, unsigned int page = 0) const
  {
	// TODO: check if valid
	if(!NBIT_BX) ibx = 0;
	return dataarray_[ibx][page][index];
  }
...
#pragma HLS inline
    ...
    if (addr_index < (1<<NBIT_ADDR)) {
      ...
      dataarray_[ibx][page][0] = data;
#else
      dataarray_[ibx][page][nentries_[ibx*NPAGE+page]++] = data;
#endif
  ...
  bool write_mem_clear(BunchXingT ibx, DataType data, NEntryT addr_index, unsigned int page)
  {
    if(!NBIT_BX) ibx = 0;
    if (addr_index < (1<<NBIT_ADDR)) {
      dataarray_[ibx][page][addr_index] = data;
      return true;
    } else {
      return false;
    }
  ...
  {
	print_data(dataarray_[bx][page][index]);
  }
