...
// Note that the masks are "reversed"
	static const int layer(1); // Which barrel layer number the data is coming from, 0 if not barrel
	static const int disk(0); // Which disk number the data is coming from, 0 if not disk
...
// SHOULD I USE SOMETHING ELSE THAN INT FOR MY TABLES???
					...
					VMRouter<BARRELPS, BARRELPS, BARRELPS, layer, disk, bendtablesize>
