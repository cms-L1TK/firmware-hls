...
#pragma HLS unroll
    ...
    if (validin_) {
      ...
      const auto slot = zbin.range(zbin.length()-1, 1);
      const ap_uint<4> nstubfirstMinus = instubdata.getEntries(bx, ivmMinus*nbins + slot);
      const ap_uint<4> nstublastMinus = zfirst==zlast ? ap_uint<4>(0) : instubdata.getEntries(bx, ivmMinus*nbins + slot + 1);
      const ap_uint<4> nstubfirstPlus = ivmMinus==ivmPlus ? ap_uint<4>(0) : instubdata.getEntries(bx, ivmPlus*nbins + slot);
      const ap_uint<4> nstublastPlus = (ivmMinus==ivmPlus || zfirst==zlast) ? ap_uint<4>(0) : instubdata.getEntries(bx, ivmPlus*nbins + slot + 1);
      
      const ap_uint<16> nstubs=(nstublastPlus, nstubfirstPlus, nstublastMinus, nstubfirstMinus);
      ...
      const ap_uint<1> useSecond = zbin.range(0,0) == 1;
      const ap_uint<1> usefirstMinus = instubdata.getEntries(bx, ivmMinus*nbins + slot) != 0;
      const ap_uint<1> usesecondMinus = useSecond && (instubdata.getEntries(bx, ivmMinus*nbins + slot + 1) != 0);
      const ap_uint<1> usefirstPlus = ivmPlus != ivmMinus && (instubdata.getEntries(bx, ivmPlus*nbins + slot) != 0);
      const ap_uint<1> usesecondPlus = ivmPlus != ivmMinus && (useSecond && (instubdata.getEntries(bx, ivmPlus*nbins + slot + 1) != 0));
