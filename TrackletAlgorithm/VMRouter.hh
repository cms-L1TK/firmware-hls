	  ...
	  if ( memask[31] ) m31->clear(bx);
		...
		TOPLEVEL: for(auto i = 0; i < kMaxProc; ++i ) {
			...
			if ( DISK ) { // Not implemented
				...
				typename VMStubME<METYPE>::VMSMEID index = (r>>(r.length()-nbitsfinebintable))&((1<<nbitsfinebintable)-1);
				...
				typename VMStubME<METYPE>::VMSMEID index = (z>>(z.length()-nbitsfinebintable))&((1<<nbitsfinebintable)-1);
			...
			if ( olmask != 0 ) {//(LAYER == 1  || LAYER == 2) { // Make sure that only layer 1 and 2 are overlapped
				...
				if (overlap != 1023) { // which is like -1 if we had signed stuff?
					...
					stubOL.setFinePhi(iphivmFineBins<INTYPE>(stubPhi, nphireg, nfinephi));  // is this the right nphireg

					std::cout << "Overlap stub " << overlap << " "<< std::hex << stubOL.raw() << std::endl;
					std::cout << "iphiRaw: " << std::dec << iphiRaw << std::endl << std::endl;
					// No coarse phi bins? Aaaah probably yes, since they correspond to the different VMs?
			...
			if ( teomask != 0) {
				...
				if ( LAYER != 0 ) {
					...
					// Get the 3 MSBs of z and add 4 as z is signed (takes values between -4 and 3)
					bin =  (z>>(z.length()-TEBinsBits)) + (1<<(TEBinsBits-1)); // Coarse z value

					// Ignoring the sign (MSB): the top 7 MSBs of z. Note: not the index that is being saved to the stub
					typename VMStubTEOuter<METYPE>::VMSTEOID index = (z>>(z.length()-nbitsfinebintable))&((1<<nbitsfinebintable)-1);
					// Set zfine: the z position within a bin
					typename VMStubTEOuter<METYPE>::VMSTEOFINEZ zfine = finebintable[index]; // Using the bits 5 down to 2?
					stubme.setFineZ(zfine);
				}
				else { // DISKS
					assert(DISK!=0);
					...
					// Get the 3 MSBs of r and add 4 as r is signed (takes values between -4 and 3)
					bin =  (r>>(r.length()-TEBinsBits)) + (1<<(TEBinsBits-1)); // Coarse r value

					// Ignoring the sign (MSB): the top 7 MSBs of r. Note, not the index that is being saved to the stub
					typename VMStubTEOuter<METYPE>::VMSTEOID index = (r>>(r.length()-nbitsfinebintable))&((1<<nbitsfinebintable)-1);
					// set rfine: the r position within a bin
					typename VMStubTEOuter<METYPE>::VMSTEOFINEZ rfine=finebintable[index]; // is it the same table as for z?
					assert(rfine>=0);
					stubme.setFineZ(rfine);
				}

				std::cout << "TEOuter stub " << std::hex << stubTeOuter.raw() << std::endl;
				std::cout << "iphiRaw: " << std::dec << iphiRaw << std::endl << std::endl;
				// No coarse phi bins? Aaaah probably yes, since they correspond to the different VMs?
