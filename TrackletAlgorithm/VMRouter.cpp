    ...
    void VMRouter(
      ...
      VMStubMEMemory<BARRELPS> *m31)
      {
        ...
        TOPLEVEL: for(auto i = 0; i < kMaxProc; ++i ) {
          ...
          auto iphiRaw = iphivmRaw(stubPhi); // Top 5 bits of phi
          ...
          if ( disk ) { // Not implemented
            ...
            VMStubME<BARRELPS>::VMSMEID index = (z>>(z.length()-nbitsfinebintable_))&((1<<nbitsfinebintable_)-1);
            // Set zfine: the z position within a bin
            VMStubME<BARRELPS>::VMSMEFINEZ zfine = finebintable_[index]; //Takes time
