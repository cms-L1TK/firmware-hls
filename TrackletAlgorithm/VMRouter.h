...
inline int iphivmFineBins(const typename AllStub<INTYPE>::ASPHI phi,
	...
	assert(v1 == v2);
	return v1; //(phi >> (phi.length() - VMbits - finebits)) & ((1 << finebits) - 1);;
}
...
template<regionType INTYPE, regionType METYPE, int LAYER, int DISK, int bendtablesize> //, int NINPUTS, uint32_t MEMASK, uint32_t TEIMASK, uint16_t OLMASK, uint32_t TEOMASK>
void VMRouter(const BXType bx, const int finebintable[], const int binlookuptable[], const int bendtable[][bendtablesize],
...
//#pragma HLS array_partition variable=bendtable // Only seemed to increase the use of LUTs
//#pragma HLS array_partition variable=binlookuptable //max 1024 partitions
//#pragma HLS array_partition variable=overlaptable // only seemed to increase the use of LUTs
...
#pragma HLS PIPELINE II=1
		...
		//auto LAYER = layer; // hack.
		//auto DISK  = disk; // hack --these are mutually exclusive so ...
		...
		//iphiRaw = iphiRawPlus;
		int iphiRawtmp = iphiRaw/(32/nvmte);
		assert(iphiRaw == iphiRawtmp);
		...
		if (teimask != 0) {
			...
			if (LAYER != 0) {
				...
				ap_uint<4> rbin = (r + (1 << (r.length() - 1))) >> (r.length() - 4); // What is this doing... r already positive?! 4 MSBs??

				int index = zbin * rbins + rbin; // number of bins

				binlookup = binlookuptable[index];
				std::cout << "r: " << r << "    bin: " << rbin <<std::endl;
			...
			bool passbend = bendtable[0][bend]; // Doesn't work if bendcut isn't the same for all regions
		...
		if (teomask != 0) {
			...
			if (LAYER != 0) {
				...
				// ap_uint<4> rbin = (r + (1 << (r.length() - 1))) >> (r.length() - 4); // What is this doing... r already positive?! 4 MSBs??
				//
				// int binindex = zbin * rbins + rbin; // number of bins
				//
				// binlookup = binlookuptable[binindex];
				// bin = binlookup/8;
				// std::cout << "r: " << r << "    bin: " << rbin <<std::endl;
				// std::cout << "INDEX: " << binindex << "    binlookup: " << binlookup <<std::endl;
			...
			bool passbend = bendtable[iphiRaw][bend];
			if (passbend) { //&& binlookup > 0) {
			// Save stub to memories
			// 0-9
			if (teomask[0]) {
				if (iphiRaw == 0)
