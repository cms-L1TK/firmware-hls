...
int main()
{
  ...
  for (int i=0; i<MAX_nEVENTS; i++)
  {
    ...
    {
      curnStubs[j] = nStubs[i*MAX_nROUTERS + j];
    }
    ...
    for (int j=0; j<MAX_nROUTERS; j++)
    {
      nPH1Z1[i*MAX_nROUTERS + j] = curnPH1Z1[j];
      nPH2Z1[i*MAX_nROUTERS + j] = curnPH2Z1[j];
      nPH3Z1[i*MAX_nROUTERS + j] = curnPH3Z1[j];
      nPH4Z1[i*MAX_nROUTERS + j] = curnPH4Z1[j];
      nPH1Z2[i*MAX_nROUTERS + j] = curnPH1Z2[j];
      nPH2Z2[i*MAX_nROUTERS + j] = curnPH2Z2[j];
      nPH3Z2[i*MAX_nROUTERS + j] = curnPH3Z2[j];
      nPH4Z2[i*MAX_nROUTERS + j] = curnPH4Z2[j];
    }
    for (int j=0; j<MAX_nROUTERS; j++)
    {
      curnPH1Z1[j] = 0;
