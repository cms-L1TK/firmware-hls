...
int main()
{
	...
	HLSCandidateMatch * inData1[nevents-1][MAX_nCM];
    ...
    while (getline(fin,token)){
		...
		// make CM
		if (cur_CM < MAX_nCM){ // only store if it is less than max number of CM
			inData1[0][cur_CM] = new HLSCandidateMatch(in_p_index,in_s_index);
			// increment counter
			cur_CM++;
		}
		else std::cout << "Hit the max number of CM allowed" << std::endl;
    }
    fin.close();
